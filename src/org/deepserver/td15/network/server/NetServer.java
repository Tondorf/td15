...
public class NetServer {
	...
	public synchronized void registerNewClient(Socket clientSocket) throws AboveCapacityException,
		...
		ClientConnection clientConnection = new ClientConnection(clientID, clientSocket,
				protocolWorker);
	...
	private void sendClient(ClientConnection clientConnection, byte[] msg) {
		try {
			...
			logger.error("Could not send message to client #" + clientConnection.getId()
					+ " (error: " + e.getMessage() + ")");
		}
	}
}